...
class Rbac
{
    ...
    public function assignUserRole($userId, $role)
    {
        ...
        return Db::name($this->userTable)->update(['id' => $userId, 'role_id' => $role]);
    }
     ...
     * 查询出该用户的所有权限并存入缓存
     */
    public function cachePermission($id, $user_id)
    {
        if (empty($id)) {
            throw new Exception('参数错误');
        }
        $roleId = db($this->userTable)->where('id', $id)->value('role_id');
        $permission = Db::name($this->permissionTable)->alias('p')->join("{$this->rolePermissionTable} rp", "p.id = rp.permission_id")
            ->where("rp.role_id = {$roleId}")->select();
        $newPermission = [];
        if (!empty($permission)) {
        ...
        cache("permission" . $user_id, $newPermission);
     ...
     * 检查用户有没有权限执行某操作
     */
    public function can($path, $user_id)
    {
        $permissionList = cache("permission" . $user_id);
